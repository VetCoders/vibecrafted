...
spawn_prepare_paths junie "$plan_file" "$root" "$mode"
spawn_scan_active "${SPAWN_LOG_DIR:-$SPAWN_REPORT_DIR}"
runtime_input="$SPAWN_TMP_DIR/${SPAWN_TS}_${SPAWN_SLUG}_junie_prompt.md"
...
launch_cmd="set -o pipefail && cd $qroot && { junie --project=$qroot --task=\"\$(cat $qruntime)\" --skip-update-check --use-local-cache --output-format=text $model_flag $review_flag $timeout_flag 2>&1 | tee -a $qtranscript; pipeline_status=\$?; exit \$pipeline_status; }"
