...
# Marbles Stop Hook
# Prevents session exit when a marbles is active
...
# Check if marbles is active
MARBLES_STATE_FILE=".claude/marbles.local.md"

if [[ ! -f "$MARBLES_STATE_FILE" ]]; then
  # No active loop - allow exit
  exit 0
fi

# Parse markdown frontmatter (YAML between ---) and extract values
FRONTMATTER=$(sed -n '/^---$/,/^---$/{ /^---$/d; p; }' "$MARBLES_STATE_FILE")
...
# Older state files without session_id fall through (preserves old behavior).
...
if [[ ! "$ITERATION" =~ ^[0-9]+$ ]]; then
  ...
  echo "   Marbles is stopping. Run /marbles again to start fresh." >&2
...
if [[ "$COMPLETION_PROMISE" != "null" ]] && [[ -n "$COMPLETION_PROMISE" ]]; then
  SYSTEM_MSG="🔄 Marbles iteration $NEXT_ITERATION | To stop: output <promise>$COMPLETION_PROMISE</promise> (ONLY when statement is TRUE - do not lie to exit!)"
else
  SYSTEM_MSG="🔄 Marbles iteration $NEXT_ITERATION | No completion promise set - loop runs infinitely"
fi
