...
var uiStrings = isPolishLocale
  ? {
      ...
      copyCodeFallback:
        "Kopiowanie się nie udało. Zaznacz polecenie i skopiuj je ręcznie.",
      installCopied: "Polecenie instalacji zostało skopiowane do schowka.",
      copyInstallFallback:
        "Kopiowanie się nie udało. Zaznacz polecenie instalacji i skopiuj je ręcznie.",
...
var MarbleFactory = (function () {
  ...
  function lighten(c, pct) {
    return {
      r: Math.min(255, c.r + pct),
      g: Math.min(255, c.g + pct),
      b: Math.min(255, c.b + pct),
    };
  }

  function darken(c, pct) {
    return {
      r: Math.max(0, c.r - pct),
      g: Math.max(0, c.g - pct),
      b: Math.max(0, c.b - pct),
    };
  }
  ...
  function createSprite(radius, paletteIdx, patternType, seed) {
    ...
    var rfl = g.createRadialGradient(
      cx + r * 0.2,
      cy + r * 0.35,
      r * 0.02,
      cx + r * 0.15,
  ...
  for (var y = cy - R + mr; y <= cy + R - mr; y += rh) {
    ...
    for (var x = cx - R + mr; x <= cx + R - mr; x += sp) {
      var px = x + off,
        dx = px - cx,
        dy = y - cy;
      if (Math.sqrt(dx * dx + dy * dy) + mr * 0.85 <= R)
        pos.push({ x: px, y: y });
    }
    row++;
  }
...
(function () {
  ...
  var dpr = Math.max(
    1,
    Math.min(window.devicePixelRatio || 1, isSafari ? 1.5 : 2),
  );
  var width = 0,
    height = 0;
  var slots = [];
      ...
      build: function (cx, cy, R, mr) {
        return buildPredicateShape(cx, cy, R, mr, function (nx, ny) {
          ...
          var r =
            0.86 *
            (0.45 +
              0.55 * Math.pow(Math.abs(Math.cos(2.5 * a - Math.PI / 2)), 0.6));
          return d <= r;
        });
      },
    },
  ...
  function throwWave() {
    ...
    if (remainingLoops > 1) {
      toThrow = Math.min(
        toThrow,
        Math.max(1, unassigned.length - reserveForLater),
      );
    } else {
    ...
    for (var i = 0; i < toThrow; i++) {
      ...
      var startY =
        board.y +
        Math.sin(angle) * spawnSpread -
        board.radius * chaosLevel * 0.5;
      var mSeed = Math.floor(Math.random() * 99999);
      var mPattern =
        MarbleFactory.PATTERNS[
          Math.floor(Math.random() * MarbleFactory.PATTERNS.length)
        ];
      ...
      marbles.push({
        ...
        pattern: mPattern,
        palIdx: mPalIdx,
        sprite: MarbleFactory.createSprite(
          marbleRadius,
          mPalIdx,
          mPattern,
          mSeed,
  ...
  function tick(now) {
    ...
    marbles = marbles.filter(function (m) {
      ...
      var inBounds =
        m.x > -100 && m.x < width + 100 && m.y > -300 && m.y < height + 100;
    ...
    marbles.forEach((m) => {
      if (!m.settled) {
        ...
        var jitter =
          dist > marbleRadius * 2.4 ? (Math.random() - 0.5) * 3.2 * mChaos : 0;
    ...
    renderList.forEach((m) => {
      ...
      var mAlpha = m.settled
        ? 0.7 + conv * 0.3
        : 0.5 + Math.min(speed * 0.03, 0.4);
      ...
      if (hover > 0 || speed > 0.5) {
        ...
        ctx.ellipse(
          m.x,
          m.y + marbleRadius * 0.6 + hover * 0.15,
          shadowW,
          shadowH,
          0,
          0,
...
(function () {
  ...
  var cw,
    ...
    currentLoop = 0,
    totalSlots = 0;
  var loopFill = [0, 0.15, 0.4, 0.65, 0.85, 1.0];
  var filled = {},
    noise = {};
  var mx = -1,
    my = -1;
  ...
  function countFilled() {
    var n = 0;
    for (var k in filled)
      if (filled[k] && slots[k].marble && !slots[k].marble.shaking) n++;
    return n;
  }
  ...
  function render() {
    ...
    for (var i = 0; i < slots.length; i++) {
      ...
      if (m.shaking) {
        ...
        var p = prox(s.x, s.y);
        ctx.globalAlpha =
          (DIM + p * (BRIGHT - DIM)) * (1 - m.shakeT * m.shakeT);
        ctx.drawImage(
          m.sprite,
          s.x + wobble - m.sprite.width / 2,
          s.y - m.sprite.height / 2,
        );
        continue;
      }
      ...
      ctx.drawImage(
        m.sprite,
        s.x - m.sprite.width / 2,
        s.y + yOff - m.sprite.height / 2,
      );
    }
    ctx.globalAlpha = 1;
    ...
    ctx.fillText(
      currentLoop > 0 ? "Loop " + currentLoop : "Board",
      cw / 2,
      cw / 2 + cw * 0.07,
    );
...
(function () {
  ...
  buttons.forEach(function (btn) {
    btn.setAttribute(
      "data-default-label",
      btn.textContent.trim() || uiStrings.copy,
    );
    vcListen(btn, "click", function () {
      ...
      copyText(text).then(function (copied) {
        ...
        announceUiMessage(
          copied ? uiStrings.codeCopied : uiStrings.copyCodeFallback,
        );
      });
    });
  });
  ...
  buttons.forEach(function (btn) {
    ...
    vcListen(btn, "click", function () {
      ...
      copyText(cmd).then(function (copied) {
        flashCopyButton(btn, copied);
        announceUiMessage(
          copied ? uiStrings.installCopied : uiStrings.copyInstallFallback,
        );
      });
    });
  });
...
(function () {
  var copyables = document.querySelectorAll(
    ".hterm-prompt[data-copy], .hterm-row[data-copy]",
  );
  ...
  function copyFromElement(el, x, y) {
    ...
    copyText(value).then(function (copied) {
      showTip(
        x,
        y,
        copied ? uiStrings.copied : uiStrings.copyFailed,
        copied ? "ok" : "err",
      );
      announceUiMessage(
        copied ? uiStrings.commandCopied : uiStrings.copyCommandFallback,
      );
...
(function () {
  ...
  function measureCycleWidth() {
    ...
    cycleWidth = originals.reduce(function (sum, card, index) {
      return (
        ...
        (index < originals.length - 1 ? gap : 0)
      );
    }, 0);
  }
  ...
  function jumpToMiddle(force) {
    ...
    targetScrollLeft = normalizeScrollLeft(
  ...
  function pushMomentum(distance, impulseMultiplier) {
    ...
    if (!cycleWidth) return;
    targetScrollLeft = normalizeScrollLeft(
      (targetScrollLeft || strip.scrollLeft) + distance,
    );
...
(function () {
  ...
  function measurePaneHeight(pane, paneWidth) {
    ...
    var measured = Math.ceil(
      pane.scrollHeight || pane.getBoundingClientRect().height || 0,
    );
  ...
  function initMermaid() {
    if (!mermaidInitialized && window.mermaid) {
      try {
        ...
        mermaid
          .render("mermaid-graph-1", rawText)
