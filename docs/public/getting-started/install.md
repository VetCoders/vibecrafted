---
title: "Install"
description: "Install Vibecrafted on macOS, Linux, or native Windows, and verify the result with doctor."
...
Vibecrafted runs on macOS, Linux, and native Windows. Pick the channel
...
| Signed Runtime Pack          | Windows 10+, x64     | Same prebuilt runtime; no WSL, no GUI required         | Native lane; publication pending         |
...
On macOS and Linux, use the bootstrap today. On Windows, install the native
Runtime Pack (`install.ps1` / `scripts/install-runtime-pack.ps1`). WSL2 remains
an optional way to run the Linux portable channel, not the native product.
...
Native Windows uses the same Runtime Pack contract as macOS/Linux: one immutable
generation, one `active.json` authority, one receipted uninstaller. The carrier
is `Vibecrafted_RuntimePack_<version>-<YYYYMMDD>-<sha8>-win32-x64.tar.gz` plus
adjacent `.sha256` and `.sig`.

```powershell
$env:VIBECRAFTED_RUNTIME_PACK = '.\Vibecrafted_RuntimePack_4.3.0-win32-x64.tar.gz'
.\install.ps1
```

From a checkout that already has the pack in `dist\`:

```powershell
.\install.ps1
```

This path does not require WSL, Homebrew, an app bundle, or a Rust/Node/Python
toolchain. `vc-server` binds to `127.0.0.1:3024` by default and is stopped by
`vibecrafted server stop` or uninstall.

WSL2 remains available if you want the Linux portable tarball inside a distro:
...
layout. Native Windows does not go through that path.
...
- It is **not** a prebuilt-binary bundle. The Rust cockpit binaries (`voc`,
  ...
  section above.
- On Windows the native product is the Runtime Pack (`install.ps1`). The
  portable tarball remains the artifact you use _inside_ WSL2 if you choose
  that optional Linux channel.
