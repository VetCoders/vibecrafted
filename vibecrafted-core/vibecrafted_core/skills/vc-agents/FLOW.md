...
| `vibecrafted <action> <agent>`              | mode-specific args                           | launcher plus report, transcript, and meta under the artifact root | `0` on dispatch |
...
- Existing run needs inspection -> `vibecrafted observe <agent> --last` or `await --last`.
