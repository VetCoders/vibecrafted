...
launch_cmd="set -o pipefail && cd $qroot && { rm -f $qlast_message; junie --project=$qroot --task=\"\$(cat $qruntime)\" --skip-update-check --use-local-cache --input-format=text --output-format=json-stream $model_flag $review_flag $timeout_flag 2>&1 | tee -a $qtranscript; pipeline_status=\$?; $last_message_extract $salvage_success_report $salvage_failure_report exit \$pipeline_status; }"
