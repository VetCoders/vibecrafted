...
def _payload_files(root: Path) -> list[dict[str, Any]]:
    ...
    for path in sorted(root.rglob("*")):
        ...
        if path.name in FORBIDDEN_PAYLOAD_NAMES:
            ...
            # than any pre-walk sweep can win the race (field: four distinct
            # build/install failures on one operator Mac in a day). The
            # carrier tar excludes these names, so they can never ship and
            # never carry signed bytes: skip them instead of failing an
            # otherwise valid payload, and never record them in provenance.
            continue
        if path.is_symlink():
            raise RuntimePackContractError(
